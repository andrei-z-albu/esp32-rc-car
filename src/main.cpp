...
#define SERVO_MAX_LEFT 130   // Increased from 120 for better steering authority
#define SERVO_MAX_RIGHT 50   // Decreased from 60 for better steering authority
...
const char* TRIP1_KEY = "trip1_m";
const char* TRIP2_KEY = "trip2_m";
...
TripPage currentPage = TOTAL_ODOMETRY;
float trip1Distance = 0.0f;        // Persistent trip 1
float trip2Distance = 0.0f;        // Persistent trip 2  
float currentTripDistance = 0.0f;  // Session-only trip (reset on power cycle)
float lastTrip1SavedDistance = 0.0f;
float lastTrip2SavedDistance = 0.0f;

// D-pad navigation state
uint16_t lastDPad = 0;
unsigned long dpadDownStartTime = 0;
...
String formatDistance(float distance) {
    if (distance >= 1000.0) {
        return String(distance / 1000.0, 1) + "km";
...
void drawPageDots() {
    // Draw pagination dots at the bottom of screen
    int regularDotSize = 3;
    int activeDotSize = 6;  // Much larger for current page
    int dotSpacing = 10;    // Increased spacing to accommodate larger dot
    int totalWidth = (NUM_PAGES * activeDotSize) + ((NUM_PAGES - 1) * dotSpacing);
    int startX = (SCREEN_WIDTH - totalWidth) / 2;
    int y = SCREEN_HEIGHT - 8;
    
    for (int i = 0; i < NUM_PAGES; i++) {
        int x = startX + (i * (activeDotSize + dotSpacing));
        if (i == currentPage) {
            // Current page - large filled circle (very bold)
            display.fillCircle(x + activeDotSize/2, y, activeDotSize/2, SSD1306_WHITE);
        } else {
            // Other pages - small hollow circle, centered within the larger space
            int offset = (activeDotSize - regularDotSize) / 2;
            display.drawCircle(x + activeDotSize/2, y, regularDotSize/2, SSD1306_WHITE);
        }
    }
}

void displayTripComputer() {
    // Clear display
    ...
    switch (currentPage) {
        ...
        case TRIP_1:
            title = "Trip 1";
            distanceText = formatDistance(trip1Distance);
            footer = "Hold DOWN to reset";
            break;
            
        case TRIP_2:
            title = "Trip 2";
            distanceText = formatDistance(trip2Distance);
            footer = "Hold DOWN to reset";
            break;
            
        case CURRENT_TRIP:
            title = "Current Trip";
            distanceText = formatDistance(currentTripDistance);
            break;
    }
...
void handleTripComputerNavigation(uint16_t dpad) {
    // Detect d-pad button press (not held)
    bool leftPressed = (dpad & leftButton) && !(lastDPad & leftButton);
    bool rightPressed = (dpad & rightButton) && !(lastDPad & rightButton);
    bool downPressed = (dpad & downButton) && !(lastDPad & downButton);
    bool downReleased = !(dpad & downButton) && (lastDPad & downButton);
    
    // Handle left/right navigation
    if (leftPressed) {
        // Navigate left (circular: 0 -> 3, 1 -> 0, 2 -> 1, 3 -> 2)
        currentPage = (TripPage)((currentPage + NUM_PAGES - 1) % NUM_PAGES);
        Serial.printf("Trip computer: navigated left to page %d\n", currentPage);
    } else if (rightPressed) {
        // Navigate right (circular: 0 -> 1, 1 -> 2, 2 -> 3, 3 -> 0)
        currentPage = (TripPage)((currentPage + 1) % NUM_PAGES);
        Serial.printf("Trip computer: navigated right to page %d\n", currentPage);
    }
    
    // Handle down button for reset (hold for 3 seconds)
    if (downPressed) {
        dpadDownStartTime = millis();
        dpadDownHeld = true;
    } else if (downReleased) {
        dpadDownHeld = false;
    }
    
    // Check for reset action (hold down for 3 seconds)
    if (dpadDownHeld && (millis() - dpadDownStartTime >= 3000)) {
        if (currentPage == TRIP_1) {
            trip1Distance = 0.0f;
            odomPrefs.putFloat(TRIP1_KEY, trip1Distance);
            lastTrip1SavedDistance = 0.0f;
            Serial.println("Trip 1 reset to 0.0m");
        } else if (currentPage == TRIP_2) {
            trip2Distance = 0.0f;
            odomPrefs.putFloat(TRIP2_KEY, trip2Distance);
            lastTrip2SavedDistance = 0.0f;
            Serial.println("Trip 2 reset to 0.0m");
        }
        dpadDownHeld = false; // Prevent multiple resets
    }
    
    // Store current d-pad state for next iteration
    lastDPad = dpad;
}

void displayOdometry() {
    // Use the new trip computer display
    displayTripComputer();
}
...
void updateOdometry() {
    ...
    if (currentTime - lastOdometryTime >= 50) {  // Update every 50ms
        ...
        float deltaDistance = abs(averageDelta);
        totalDistance += deltaDistance;
        trip1Distance += deltaDistance;
        trip2Distance += deltaDistance;
        currentTripDistance += deltaDistance;
        ...
        if ((trip1Distance - lastTrip1SavedDistance) > 1.0f) shouldSave = true; // trip1 distance based
        if ((trip2Distance - lastTrip2SavedDistance) > 1.0f) shouldSave = true; // trip2 distance based
        ...
        if (shouldSave) {
            ...
            lastTrip1SavedDistance = trip1Distance;
            lastTrip2SavedDistance = trip2Distance;
            Serial.printf("Odometry saved - Total: %.2f m, Trip1: %.2f m, Trip2: %.2f m\n", 
...
void setServoAngle(int angle) {
    ...
    // Apply safe range constraints to prevent mechanical binding
    angle = constrain(angle, SERVO_MAX_RIGHT, SERVO_MAX_LEFT);
    
    // Additional safety check - never go beyond 30-150 degree range
    ...
    // Debug output for servo angle changes
...
void setup() {
    ...
    if (!odomPrefs.begin("odom", false)) { // false means don't erase old data
        ...
        trip1Distance = odomPrefs.getFloat(TRIP1_KEY, 0.0f); // Load trip 1 distance
        trip2Distance = odomPrefs.getFloat(TRIP2_KEY, 0.0f); // Load trip 2 distance
        // currentTripDistance starts at 0.0f (not persistent)
